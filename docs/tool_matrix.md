...
| `get_project_status` | yes | `project-status` | yes | unit tests only | implemented | Project goals, decisions, open questions, rejected ideas |
...
`latest-runs` / `compare-runs` / `project-status` subcommands) remains available and delegates
to the same engine.
