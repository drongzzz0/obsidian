...
- 导入命令默认 dry-run；只有显式传入 `--write` 才写库。
...
- 完整 synthetic 写入 demo 已经端到端测试：`schema-check`、`import-bibtex --write`、
...
- 包可本地安装，但尚未发布到 PyPI。
- 论文元数据、curated notes 和实验 run 入库已有显式 CLI 路径，但私有数据库写入仍必须
  传入 `--write`；`schema-check` 可以在写入前发现缺表或缺字段。
