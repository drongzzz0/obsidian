...
def default_eval_file() -> Path:
    cwd_candidate = Path("evals") / "retrieval_eval.jsonl"
    if cwd_candidate.exists():
        return cwd_candidate
    packaged = Path(__file__).resolve().parents[2] / "evals" / "retrieval_eval.jsonl"
    return packaged if packaged.exists() else cwd_candidate
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument(
        ...
        type=Path,
        default=None,
        help="Gold JSONL query set. Defaults to ./evals/retrieval_eval.jsonl, falling back to the repository copy.",
    )
    ...
    cases = load_cases(args.eval_file or default_eval_file())
